...
(defun cpd--composiphrase-execute-advice (orig-fun sentence config)
  ...
  (let* ((verb-word (seq-find (lambda (word) (eq 'verb (cdr (assq 'word-type word))))
    ...
    (unless is-movement
      (if estate--visual-clamped-modifier-active
...
;;; Visual clamped modifier mode integration with composiphrase.

(require 'estate-visual-modifier-clamped-state)

(defun cpd--thing-to-clamped-bounds-func (thing)
  "Convert a THING symbol into a bounds function for clamped modifier mode.
Returns a function of zero arguments that returns (BEG . END) at point."
  (lambda ()
    (bounds-of-thing-at-point thing)))

(defun cpd-estate-visual-clamped-modifier-basic-activate (thing)
  "Activate visual clamped modifier mode for a basic THING type.
...
(defun cpd-estate-visual-clamped-modifier-tree-activate (tree-name tree-bounds-func)
  "Activate visual clamped modifier mode for a tree-based object.
