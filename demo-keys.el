...
(defhydra cpo/object-select (:foreign-keys warn :exit nil) "Obj:"
  ...
  ("h SPC" (lambda (n) (interactive "p")
   ...
   "whitespace" :exit t)
  ("hu" (lambda (n) (interactive "p")
          (funcall (cp/ae (progn (require 'cpo-url-object) (cp/obj 'url))) n))
   "url" :exit t)
  ("hd" (lambda (n) (interactive "p")
          (funcall (cp/ae (progn (require 'cpo-date-object) (cp/obj 'date-yyyy-mm-dd))) n))
   "date" :exit t)
  ("he" (lambda (n) (interactive "p")
          (funcall (cp/ae (progn (require 'cpo-url-object) (cp/obj 'email))) n))
   "email" :exit t)
  ("hP" (lambda (n) (interactive "p")
