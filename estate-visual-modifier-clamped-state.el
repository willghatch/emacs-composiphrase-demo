...
(defvar-local estate--visual-clamped-modifier-func nil
...
(defvar-local estate--visual-clamped-modifier-name nil
  "Name of the current clamped modifier type, for display purposes.")

(defvar-local estate--visual-clamped-modifier-active nil
  "Non-nil when visual clamped modifier mode is active.")
...
(defvar-local estate--visual-clamped-modifier-overlays nil
  "List of overlays showing the clamped modifier region.")
...
(defvar estate-visual-clamped-modifier-command-predicate nil
...
(defun estate--visual-clamped-modifier-bounds-at-point ()
...
transitions caused by mark/region changes during bounds computation."
  (when estate--visual-clamped-modifier-func
    (unwind-protect
        (progn
          (estate--deactivate-initialize-core-states)
          (funcall estate--visual-clamped-modifier-func))
      (estate--activate-initialize-core-states))))

;; Overlay management

(defun estate--visual-clamped-modifier-update-overlay ()
  "Update the overlay showing the text object region at point."
  (when estate--visual-clamped-modifier-active
    ;; Clean up existing overlays
    (dolist (ov estate--visual-clamped-modifier-overlays)
      (delete-overlay ov))
    (setq-local estate--visual-clamped-modifier-overlays nil)
    ;; Create overlay for the text object at point
    (let ((bounds (estate--visual-clamped-modifier-bounds-at-point)))
      (when bounds
        (let ((ov (make-overlay (car bounds) (cdr bounds))))
          (overlay-put ov 'face 'estate-visual-modifier-expanded-region-face)
          (overlay-put ov 'priority 100)
          (push ov estate--visual-clamped-modifier-overlays))))))

;; Region expansion (for non-movement actions)

(defun estate--visual-clamped-modifier-do-expansion ()
  "Expand the region to the text object boundaries at point.
Returns non-nil if expansion was performed."
  (when (and estate--visual-clamped-modifier-active
...
(defun estate-visual-clamped-modifier-expand-region ()
...
during the pre-command hook."
  (estate--visual-clamped-modifier-do-expansion))

;; Hook functions

(defun estate--visual-clamped-modifier-after-change (_beg _end _len)
...
(defun estate--visual-clamped-modifier-pre-command ()
  "Pre-command hook for visual clamped modifier mode.
...
expands the region for non-movement commands."
  ...
  (when (and estate-visual-clamped-modifier-command-predicate
             (funcall estate-visual-clamped-modifier-command-predicate this-command))
    (estate--visual-clamped-modifier-do-expansion)))

(defun estate--visual-clamped-modifier-post-command ()
  "Post-command hook for visual clamped modifier mode.
  ...
  (when (and estate--visual-clamped-modifier-active
    ...
    (cond
     ...
     ((and estate--visual-clamped-region-expanded
      ...
      (estate--visual-clamped-modifier-update-overlay))
...
(defun estate-visual-state-activate-clamped-modifier (name bounds-func)
  "Activate visual clamped modifier mode.
...
for the text object at point, or nil if none."
  (when (not (eq estate-state 'visual))
    (estate-visual-state))
  (setq-local estate--visual-clamped-modifier-name name)
  (setq-local estate--visual-clamped-modifier-func bounds-func)
  (setq-local estate--visual-clamped-modifier-active t)
  (setq-local estate--visual-clamped-command-modified-buffer nil)
  (setq-local estate--visual-clamped-region-expanded nil)
  (setq-local estate--visual-clamped-original-point nil)
  (setq-local estate--visual-clamped-original-mark nil)
  ...
  (setq deactivate-mark nil)
  ;; Install hooks
  (add-hook 'pre-command-hook #'estate--visual-clamped-modifier-pre-command nil t)
  (add-hook 'post-command-hook #'estate--visual-clamped-modifier-post-command nil t)
  (add-hook 'after-change-functions #'estate--visual-clamped-modifier-after-change nil t)
  ;; Show initial overlay for the text object at point
  (estate--visual-clamped-modifier-update-overlay))

(defun estate--visual-clamped-modifier-deactivate ()
  "Deactivate visual clamped modifier mode."
  (when estate--visual-clamped-modifier-active
    (setq-local estate--visual-clamped-modifier-active nil)
    (setq-local estate--visual-clamped-modifier-func nil)
    (setq-local estate--visual-clamped-modifier-name nil)
    ...
    (setq-local estate--visual-clamped-modifier-overlays nil)
    ;; Remove hooks
    (remove-hook 'pre-command-hook #'estate--visual-clamped-modifier-pre-command t)
    (remove-hook 'post-command-hook #'estate--visual-clamped-modifier-post-command t)
    (remove-hook 'after-change-functions #'estate--visual-clamped-modifier-after-change t)))

;; Clean up on visual state exit.
(add-hook 'estate-visual-state-exit-hook #'estate--visual-clamped-modifier-deactivate)

(provide 'estate-visual-modifier-clamped-state)
